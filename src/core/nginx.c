    ...
    for (i = 1; i < argc; i++) {
        ...
        while (*p) {
            ...
            switch (*p++) {
                ...
                case '?':
                case 'h':ngx_show_version = 1;
                    ngx_show_help = 1;
                    break;

                case 'v':ngx_show_version = 1;
                    break;
...
void test_array() {
    ...
    ngx_array_t *ngx_array = ngx_array_create(ngx_pool, sizeof(int), 3);


    // 分配数组的一个新元素
    int *elem = ngx_array_push(ngx_array);
    if (elem != NULL) {
        elem[0] = 10;  // 将值赋给数组中的新元素
    }
